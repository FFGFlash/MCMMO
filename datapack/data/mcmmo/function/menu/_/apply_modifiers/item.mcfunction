...
data modify storage mcmmo:temp Compare set from storage mcmmo:temp Modifier
scoreboard players set #c1 mcmmo.menu 0
execute store success score #c1 mcmmo.menu run data modify storage mcmmo:temp Compare set from storage mcmmo:temp Items[0].components.minecraft:custom_data.menu_modifiers[-1].id

# Continue the loop if the last modifier is still not the initial modifier
execute if score #c1 mcmmo.menu matches 1 run return run function mcmmo:menu/_/apply_modifiers/item
return 1